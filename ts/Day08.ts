...
let midInput: string = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |fdgacbe cefdb cefbgd gcbe
...
let shortPut: string = `acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab |cdfeb fcadb cdfeb cdbaf`;

console.log(p2(shortPut.split(`\n`)));

function p1(inputLines: string[]): number {

    let count = 0;

    for (let i = 0; i < inputLines.length; i++) {
        
        let digits: string[] = inputLines[i].split(`|`); 
        const outputValues: string[] = digits[1].split(` `);

        for (let j = 0; j < outputValues.length; j++) {
            const segment = outputValues[j];
...
function p2(inputLines: string[]): number {
    ...
    for (let i = 0; i < inputLines.length; i++) {
        ...
        const inputValues: string[] = digits[0].split(` `);
        
        let sortedInputs: string[] = [];
        inputValues.forEach(element => {
            let sortedAlphString = element.split('').sort().join('');
            sortedInputs.push(sortedAlphString)
        });
        
        let letterToPositionMap: Map<number, Set<string>> = decodeInputs(sortedInputs); 
        
        console.log(letterToPositionMap);
        
        
        const outputValues: string[] = digits[1].split(` `);
        
        let fullSegmentSum: string = ``;
        for (let j = 0; j < outputValues.length; j++) {
            const segment = outputValues[j];
            switch(segment.length) {
                case 2:
                    // number 1
                    fullSegmentSum = fullSegmentSum + `1`;
                    break;
                case 3:
                    // number 7
                    fullSegmentSum = fullSegmentSum + `7`;
                    break;
                case 4:
                    // number 4
                    fullSegmentSum = fullSegmentSum + `4`;
                    break;
                case 5: case 6:
                    fullSegmentSum = fullSegmentSum + decodeSegment(segment);
                    break;
                case 7:
                    // number 8
                    fullSegmentSum = fullSegmentSum + `8`;
                    break;
            }
        }
        count += parseInt(fullSegmentSum);
    }
    
    return count;
}

function decodeInputs(input: string[]): Map<number, Set<string>> {

    // e.g. 1st position is top-middle
    let positionToLetterMap: Map<number, Set<string>> = new Map();
    
    for (let i in input) {
        const segment = input[i];
        switch (segment.length) {
            case 2:
                let setOf2 = new Set<string>();
                setOf2.add(segment);
                positionToLetterMap.set(1, setOf2);
                break;
            case 3:
                let setOf7 = new Set<string>();
                setOf7.add(segment);
                positionToLetterMap.set(7, setOf7);
                break;
            case 4:
                let setOf4 = new Set<string>();
                setOf4.add(segment);
                positionToLetterMap.set(4, setOf4);                
                break;
            case 5:
                let length5Options = [2, 3, 5];
                for (let option in length5Options) {
                    let curr = positionToLetterMap.get(length5Options[option]);
                    if (curr == undefined) {
                        curr = new Set();
                    } 
                    curr.add(segment);
                    positionToLetterMap.set(length5Options[option], curr);
                }
                break;
            case 6:
                let length6Options = [0, 6, 9];
                for (let option in length6Options) {
                    let curr = positionToLetterMap.get(length6Options[option]);
                    if (curr == undefined) {
                        curr = new Set();
                    } 
                    curr.add(segment);
                    positionToLetterMap.set(length6Options[option], curr);
                }
                break;
            case 7:
                let setOf8 = new Set<string>();
                setOf8.add(segment);
                positionToLetterMap.set(8, setOf8);       
                break;
            default:
                break;
        }
    ...
    let possibilitiesMap: Map<number, string[]> = new Map();
    for (let i = 1; i <= 9; i++) {
        possibilitiesMap.set(i, []);
    }

    possibilitiesMap.set(1, deriveSegment(0, positionToLetterMap));
    possibilitiesMap.set(2, deriveSegment(1, positionToLetterMap));

    console.log(possibilitiesMap);


    return positionToLetterMap;
}

function deriveSegment(segmentPosition: number, positionToSegmentsMap: Map<number, Set<string>>): string[] {

    if (segmentPosition == 0 && positionToSegmentsMap.has(1) && positionToSegmentsMap.has(7)) {
        // position 0 is diff between 7 and 1
        let segment7 = positionToSegmentsMap.get(7);
        let segment1 = positionToSegmentsMap.get(1);

        if (segment7 != undefined && segment1 != undefined) {
            return deriveCharacterDiff(segment7, segment1);
        }
    } else if (segmentPosition == 1 && positionToSegmentsMap.has(9) && positionToSegmentsMap.has(3)) {
        // position 1 is diff between 9 and 3
        let segment9 = positionToSegmentsMap.get(9);
        let segment3 = positionToSegmentsMap.get(3);

        if (segment9 != undefined && segment3 != undefined) {
            return deriveCharacterDiff(segment9, segment3);
        }

    } else if (segmentPosition == 5) {
        // position 5 is diff between 6 and 5
    }

    return [];
}

function deriveCharacterDiff(set1: Set<string>, set2: Set<string>): string[] {
    let options: string[] = []
    for (let item of set1.keys()) {
        let outerChars = item.split('');
        for (let innerItem of set2.keys()) {
            let innerChars = innerItem.split('');
            // console.log(`${outerChars} vs ${innerChars}`);
            outerChars.filter(c => !innerChars.includes(c)).forEach(c => options.push(c));
        }
    }
...
function decodeSegment(segment: string): number {
    ...
    let sortedSegment = segment.split('').sort().join('');

    switch (sortedSegment) {
        case 'abcdf':
            return 3;
        case 'bcdef':
            return 5;
